...
void main() {
  setUpAll(() async {
    ...
    final dbPath =
        '${await databaseFactory.getDatabasesPath()}/deepfry_consume_test.db';
    final file = File(dbPath);
    if (file.existsSync()) file.deleteSync();
    await LocalDB().init(path: dbPath);
  });

  Future<int> makePlan() => LocalDB().addWeeklyPlan(WeeklyPlan(weekStart: 1));
  ...
  test('情况一：冰箱足够则从冰箱扣除，归零删除', () async {
    ...
    await LocalDB().addShoppingItems([
      ShoppingItem(planId: planId, name: '番茄', amount: 2, unit: '个'),
    ]);

    await LocalDB().consumeForCooking(planId, [
  ...
  test('情况二：冰箱没有则从采购清单扣除，归零删除', () async {
    final planId = await makePlan();
    await LocalDB().addShoppingItems([
      ShoppingItem(planId: planId, name: '土豆', amount: 5, unit: '个'),
    ]);

    await LocalDB().consumeForCooking(planId, [
      (name: '土豆', amount: 5, unit: '个'),
    ]);
  ...
  test('情况三：冰箱部分则扣光冰箱，不足部分从采购扣除', () async {
    ...
    await LocalDB().addShoppingItems([
      ShoppingItem(planId: planId, name: '番茄', amount: 4, unit: '个'),
    ]);
    ...
    final shop = await LocalDB().getShoppingItems(planId);
    expect(
      shop.where((s) => s.name == '番茄').single.displayQuantity,
      '2个',
    ); // 4 - (3-1)
  });
  ...
  test('「适量」非数值：用完当前可用整条', () async {
    ...
    await LocalDB().addShoppingItems([
      ShoppingItem(planId: planId, name: '番茄', amount: 3, unit: '个'),
    ]);

    await LocalDB().consumeForCooking(planId, [
      (name: '番茄', amount: 0, unit: '适量'),
    ]);
